...
def background_func():
    ...
    for event in events:
        ...
        if time_5_min_from_now>=timezone.localize(event.scheduler_time) and event.five_min_reminded == False:
        ...
        if time_1_hour_from_now>=timezone.localize(event.scheduler_time) and event.one_hour_reminded == False:
            user_name=event.user_of_this_event.name
